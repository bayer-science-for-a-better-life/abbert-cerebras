...
def get_oas_vocab(dummy_vocab_size=None):
    """
...
def get_params(params_file, mode=None):
    ...
    vocab_size = len(get_oas_vocab(params["train_input"].get("dummy_vocab_size"))[0])
    ...
    if "predict_input" in params:
        params["predict_input"]["vocab_size"] = vocab_size
    else:
        params["predict_input"] = deepcopy(params["train_input"])

    params["predict_input"]["fw_masked_lm_prob"] = params["train_input"].get("fw_masked_lm_prob", None)
    params["predict_input"]["cdr_masked_lm_prob"] = params["train_input"].get("cdr_masked_lm_prob", None)
    ...
    params["predict_input"]["scale_mlm_weights"] = params["predict_input"].get(
