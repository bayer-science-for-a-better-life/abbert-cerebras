...
class BayerBertModel(BaseModel):
    ...
    def __init__(self, params, encode_only=False):
        ...
        vocab_size = params["predict_input"].get("vocab_size")
        if not vocab_size:
        ...
        all_encoder_outputs = params["model"]["all_encoder_outputs"]
        ...
        predict_batch_size = params["predict_input"]["batch_size"]
    ...
    def build_model(self, features, mode):
        ...
        last_encoder_output, all_encoder_layer_outputs = self.encoder(
            encoder_input,
        ...
        if self.use_pre_normalization:
            last_encoder_output = self.post_encoder_layer_norm(last_encoder_output)
            all_encoder_layer_outputs[-1] = last_encoder_output

        padding_mask = 1 - tf.cast(
            features["input_mask"], dtype=last_encoder_output.dtype
        )
        nsp_output = (
            self.nsp_layer(
                last_encoder_output, padding_mask=padding_mask, training=is_training,
            )
        ...
        if all_encoder_layer_outputs:
            encoder_output = all_encoder_layer_outputs
        else:
            encoder_output = last_encoder_output
    ...
    def build_eval_metric_inputs(self, model_outputs, labels, features):
        ...
        cast_mlm_output = tf.cast(mlm_output + 1e-5, tf.float32)
        cast_mlm_pred = tf.argmax(
            cast_mlm_output, axis=-1, output_type=tf.int32
        )
        mlm_pred = tf.cast(cast_mlm_pred, tf.float32)
    ...
    def build_eval_metric_ops(self, eval_metric_inputs, labels, features):
        ...
        metrics_dict = dict()
        if not self.disable_nsp:
            nsp_pred = tf.argmax(nsp_output, axis=-1)
        ...
        unscaled_mlm_loss = tf.reduce_sum(
            input_tensor=tf.cast(
                mlm_xentr * features["masked_lm_weights"], tf.float32,
            )
        )
        num_masked = tf.reduce_sum(features["masked_lm_weights"])
        ...
        metrics_dict["eval/confusion_matrix"] = streaming_confusion_matrix(labels, mlm_pred, num_classes, weights)
