...
from abbert2.vendored.cerebras_modelzoo.common.optimizers.AdamWOptimizer import AdamWOptimizer
from abbert2.vendored.cerebras_modelzoo.common.optimizers.GradAccumOptimizer import GradAccumOptimizer
from abbert2.vendored.cerebras_modelzoo.common.optimizers.LossScale import (
    CSDynamicLossScale,
    wrap_optimizer,
)
try:
    from tensorflow.compat.v1.train.experimental import DynamicLossScale
except ImportError:  # SANTI: where did compat go?
...
class Trainer:
    ...
    def __init__(self, params, tf_summary=False, mixed_precision=False):
        ...
        if isinstance(self._loss_scaling_factor, str):
            if self._loss_scaling_factor not in ['dynamic', 'tf_dynamic']:
                raise ValueError(
                    "Loss scaling factor must be either numeric or "
                    "one of the string values ['dynamic, 'tf_dynamic']"
                )
        elif self._weight_decay_rate != 0.0 and self._optimizer_type == "adamw":
            raise ValueError(
                "When using the AdamW optimizer with weight decay, "
                "set the loss_scaling_factor to dynamic."
            )
    ...
    def build_optimizer(self):
        ...
        if self.uses_dynamic_loss_scaling():
            if self._loss_scaling_factor == 'dynamic':
                # Explicit CS-1 optimized dynamic loss scaling
                ...
                # For any CS-1 run, DynamicLossScale will be
